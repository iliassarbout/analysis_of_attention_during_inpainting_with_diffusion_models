...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps, use_scale):
    ...
    elif schedule_name == "cosine": #!modif : added to handle cosine schedule
...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        if model_kwargs['starting_point'] is not None: #!modif ajouter un point de départ
            image_after_step = model_kwargs['starting_point']['x'].to(device)
            cur_step = model_kwargs['starting_point']['step']
        else:
            cur_step = None
        ...
        if conf.schedule_jump_params:
            ...
            time_pairs = time_pairs if model_kwargs['stop_to'] is None else time_pairs[:model_kwargs['stop_to']]
            time_pairs = time_pairs if cur_step is None else time_pairs[cur_step:]
            ...
            for t_last, t_cur in time_pairs:
                ...
                if t_cur < t_last:  # reverse
                    ...
                    pred_xstart = out["pred_xstart"]
                    yield out #!modif : allow to return even when processing forward diffusion. It allows us to keep every samples if conf.intermediate_steps.
        else: #!modif: No spaceddif/no jump (in case the parameters for jump is not into the yml conf file)
