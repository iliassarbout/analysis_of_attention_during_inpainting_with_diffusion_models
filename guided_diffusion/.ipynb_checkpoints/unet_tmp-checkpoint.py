...
class AttentionBlock(nn.Module):
    ...
    def _forward(self, x):
        ...
        h,rel_att = self.attention(qkv)
        self.last_att = rel_att
        h = self.proj_out(h)
...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv):
        ...
        rel_att = th.clone(weight.mean(dim=0))
        a = th.einsum("bts,bcs->bct", weight, v)
        return a.reshape(bs, -1, length),rel_att
...
class SuperResModel(UNetModel_tmp):
    """
