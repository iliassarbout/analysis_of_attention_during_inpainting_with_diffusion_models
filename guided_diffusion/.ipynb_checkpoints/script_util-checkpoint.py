...
from .unet_tmp import UNetModel_tmp

from .unet_id import UNetModelID, SuperResModelID
from .unet_gd import UNetModelGD
...
def create_model(
    ...
    if conf.unet_impl=="ID": #!modif no conf given, see difference between implementations in unet.py and unet_id.py
        return UNetModelID(
        in_channels=3,
    ...
    elif conf.unet_impl=="attention":
        return UNetModel_tmp(
        image_size=image_size,
        in_channels=3,
        model_channels=num_channels,
        out_channels=(3 if not learn_sigma else 6),
...
#!modif Added super res models utils
...
def sr_create_model(
    ...
    return SuperResModelID( #!modif : ID implementation of superres
        in_channels=3,
        model_channels=num_channels,
        out_channels=(3 if not learn_sigma else 6),
        num_res_blocks=num_res_blocks,
